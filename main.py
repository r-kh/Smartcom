...
def decrypt_password(_: str) -> str:
    """
    В проекте пароль извлекается из базы данных и расшифровывается,
    Но в задаче нужна реализация другого функционала, поэтому эта функция не реализована.
    (пароль для подключения к SFTP серверу берется напрямую из переменной окружения)
    """
