...
---
**files**
- "таблица текущих состояний файлов", тут фиксируются все файлы, с которыми работает система, включая источник, статус, ошибки и путь в хранилище
...
**file_events** — журнал событий по каждому файлу
...
1. core/config - Модуль с настройками и инициализацией общих компонентов (БД, SFTP, MinIO, RabbitMQ, Celery), которые используются в остальных модулях.


2. database – Модуль отвечает за локальное хранение данных для приложения
   (информация о серверах, файлах, их статусах, история обработки и т.п.)


3. sftp_scanner - Модуль для отслеживание появления новых файлов на SFTP-серверах
...
7. tasks – Модуль описания задач Celery и пайплайны обработки
   (координирует скачивание, загрузку в MinIO, отправку уведомлений и обновление статуса в БД; управляет ретраями, параллельной обработкой и очередями).
